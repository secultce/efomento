...
<template>
    <div>
        ...
        <v-row no-wrap class="overflow-x-auto mt-1">
            <v-col v-for="phase in phases" :key="phase.title" cols="auto">
                <v-card
                    variant="outlined"
                    class="mx-auto !p-2 cursor-pointer transition-all"
                    rounded="lg"
                    :disabled="phase.total === 0"
                    :class="selectedPhase === phase.value ? '!bg-[#008344FF] !border-[#008344]' : '!border-[#ccccccFF]'"
                    @click="select(phase)"
                >
                    <v-card-title
                    ...
                    <v-card-subtitle
                        class="!text-xs"
                        :class="selectedPhase === phase.value ? '!text-white' : '!text-[#1a1a1aFF]'"
                    >
