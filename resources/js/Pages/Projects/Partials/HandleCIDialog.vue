...
const handleCI = () => {
    saveCI(
        ...
        },
        {
            ...
            onError: (errors) => {
                const message =
                    Object.values(errors).flat().join(', ') || 'Ocorreu um erro ao criar a comunicação interna';
...
<template>
    <v-dialog v-model="isOpen" max-width="805" :retain-focus="false" persistent>
        <v-card class="rounded-lg d-flex flex-column" height="654">
            <v-card-title class="font-weight-bold flex-shrink-0"
                >Crie um documento de comunicação interna (CI)</v-card-title
            >
