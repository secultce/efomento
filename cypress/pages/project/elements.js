export const elements = {
    projectList: '[data-cy=project-list-table]',
    filterProjectPhaseCard: '[data-cy=filter-phase-card]',
    openProjectOpeningTabButton: '[data-cy=open-project-opening-tab-button]',
    checkboxProjectList: '[data-cy=checkbox-project-list]',
    findProjectPageInput: '[data-cy=find-project-input]',
    projectNupProjectList: '[data-cy=project-nup-project-list]',
    rowTableProjectList: '[data-cy=project-list-row-table]',
    chipProjectList: '[data-cy=chip-project-list]',
    agentNameProjectList: '[data-cy=agent-name-project-list]',
    noDataAvailableMessage: '[data-cy=no-data-available-message]',
    projectPhase: '[data-cy=project-phase]',
    createCIButton: '[data-cy=create-ci-button]',
    editCIButton: '[data-cy=edit-ci-button]',
    createBudgetOrderButton: '[data-cy=create-budget-order]',
    editBudgetOrderButton: '[data-cy=edit-budget-order]',
    createDocumentButon: '[data-cy=create-document-button]',
    editDocumentButon: '[data-cy=edit-document-button]',
    saveDocumentButton: '[data-cy=save-document-button]',
    cancelDocumentButton: '[data-cy=cancel-document-button]',
    successAlert: '.v-snackbar',
};

// Constantes de timeout para melhor manutenibilidade
export const TIMEOUTS = {
    DEFAULT: 5000,
    LONG: 8000,
    SEARCH: 3000,
};
