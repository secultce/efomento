...
class ListProjects {
    ...
    acessarPaginaDeProjetosDoFirstEdital() {
        cy.visit('/editais')
        cy.get(el.btnAcessarProjetos)
            .not('[disabled]')
            .first()
            .click()
        cy.url()
            .should('include', '/projetos')
    }

    selecionarPrimeiroItem() {
        cy.get(el.checkboxItem)
            .not('[disabled]')
            .first()
            .click()
    }
    ...
    verificarBotaoConferirDesabilitado() {
        cy.get(el.btnConferirDocumentos)
            .should('be.disabled')
    }
